...
export default function FindJobsPage() {
  ...
  const apply = async (jobId: string) => {
    if (!user) { openAuth('login', 'worker'); return; }
    const res = await fetch('/api/applications', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jobId, proposedPrice: '₹650', message: 'Available to help!', estimatedArrival: '45 min' }),
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="pt-24 pb-12 mx-auto max-w-7xl px-4">
        ...
        <p className="text-slate-500 mb-6">Discover local opportunities matching your skills and location</p>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            {jobs.map((job) => (
              <div key={job.id} className="bg-white rounded-xl border border-slate-100 p-5 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-slate-900">{job.title}</h3>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${job.urgency === 'emergency' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>{job.urgency}</span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => apply(job.id)} className="px-4 py-2 bg-teal-600 text-white rounded-xl text-sm font-medium hover:bg-teal-700">Apply for Job</button>
                </div>
              </div>
            ))}
          </div>
          <MapView center={[coords.lat, coords.lng]} markers={jobMarkers} radiusKm={15} height="600px" />
        </div>
      </div>
    </div>
  );
