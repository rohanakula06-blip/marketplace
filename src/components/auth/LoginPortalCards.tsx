...
export function LoginPortalCards({ className, compact = false, dark = false }: LoginPortalCardsProps) {
  const { t } = useTranslation();

  return (
    <div className={cn('space-y-3', className)}>
      {!compact && (
        <p className={cn('text-sm text-center sm:text-left', dark ? 'text-slate-400' : 'text-slate-500')}>
      ...
      <div className={cn('grid gap-3', compact ? 'sm:grid-cols-2' : 'sm:grid-cols-2')}>
        ...
        >
          ...
          <div className="min-w-0 text-left">
            <p className={cn('font-semibold truncate', dark ? 'text-white group-hover:text-blue-300' : 'text-slate-900 group-hover:text-blue-700')}>
              {t('dashboard.loginUser')}
            </p>
            <p className={cn('text-xs truncate', dark ? 'text-slate-400' : 'text-slate-500')}>{t('dashboard.loginUserDesc')}</p>
          </div>
          <LogIn size={16} className={cn('ml-auto shrink-0 opacity-70 group-hover:opacity-100', dark ? 'text-blue-400' : 'text-blue-500')} />
        </Link>
        ...
        >
          ...
          <div className="min-w-0 text-left">
            <p className={cn('font-semibold truncate', dark ? 'text-white group-hover:text-teal-300' : 'text-slate-900 group-hover:text-teal-700')}>
              {t('dashboard.loginPro')}
            </p>
            <p className={cn('text-xs truncate', dark ? 'text-slate-400' : 'text-slate-500')}>{t('dashboard.loginProDesc')}</p>
          </div>
          <LogIn size={16} className={cn('ml-auto shrink-0 opacity-70 group-hover:opacity-100', dark ? 'text-teal-400' : 'text-teal-500')} />
        </Link>
      </div>
    </div>
  );
