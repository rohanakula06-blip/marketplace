...
export function ActionCards({ variant = 'default', theme = 'default' }: { variant?: 'default' | 'hero'; theme?: 'default' | 'teak' }) {
    ...
    <div className="flex flex-col sm:flex-row gap-4 max-w-2xl">
      <button
        ...
        className={cn(
          ...
          isHero
            ? isTeak
              ? 'bg-teak-600 text-teak-50 shadow-lg shadow-teak-900/20 hover:bg-teak-700 hover:-translate-y-1'
      ...
      >
        <div className={cn('p-3 rounded-xl', isHero ? (isTeak ? 'bg-teak-700/30' : 'bg-black/15') : 'bg-white/20')}>
          <Search size={24} />
        </div>
        <div>
          <p className="font-bold text-lg">{t('action.findWorker')}</p>
          <p className={cn('text-sm', isHero ? (isTeak ? 'text-teak-100' : 'text-slate-800/80') : 'text-blue-100')}>{t('action.findWorkerDesc')}</p>
        </div>
      </button>
      ...
      <button
        ...
        className={cn(
          ...
          isHero
            ? isTeak
              ? 'border-2 border-teak-300 bg-white/90 text-teak-900 shadow-md hover:border-teak-500 hover:bg-white hover:-translate-y-1'
      ...
      >
        <div className={cn('p-3 rounded-xl', isHero ? (isTeak ? 'bg-teak-100' : 'bg-white/10') : 'bg-blue-50')}>
          <Briefcase size={24} className={isHero ? (isTeak ? 'text-teak-700' : 'text-amber-300') : 'text-blue-600'} />
        </div>
        <div>
          <p className="font-bold text-lg">{t('action.findWork')}</p>
          <p className={cn('text-sm', isHero ? (isTeak ? 'text-teak-600' : 'text-slate-300') : 'text-slate-500')}>{t('action.findWorkDesc')}</p>
        </div>
      </button>
    </div>
  );
